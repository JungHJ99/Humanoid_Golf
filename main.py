                ...
                if delay == 0:
                    ...
                    if status == 0:         # 0: Finding Ball
                        ...
                        if ball_angle in [-0, -15, 30, -45] and motion_cnt >= 0:    # 공의 높이에 높이에 맞추어 정면을 보고 몸을 왼쪽으로 회전함.
                            motion_dict = {
                                -0: [7, 25, 38],                                    # {key : value}: {얼굴 각도 : [왼쪽으로 몸 30도 회전 <- 왼쪽으로 몸 60도 회전 <- 홀 높이의 정면 보기]}
                                -15: [7, 25, 39], 
                                -30: [7, 25, 40],
                                -45: [7, 25, 41]
                            }
                            ...
                            if TX_num == 34:            # 34 : 머리 왼쪽 90도 하향 0도
                                if hole_detected:
                                    ball_angle = -0     # 공을 찾음 -> 다음 사이클에서 ball_angle = -0 에 맞추어 3개의 동작 실행
                                    motion_cnt = 2      
                                else:
                                    TX_num = 35         # 공을 못찾음 -> 다음 사이클에서 고개 더 내림 (하향 0도 -> 하향 15도)
                                    delay = 5
                            elif TX_num == 35:          # 35 : 머리 왼쪽 90도 하향 15도
                                if hole_detected:
                                    ball_angle = -15    # 공을 찾음 -> 다음 사이클에서 ball_angle = -15 에 맞추어 3개의 동작 실행
                                    motion_cnt = 2
                                else:
                                    TX_num = 36         # 공을 못찾음 -> 다음 사이클에서 고개 더 내림 (하향 15도 -> 하향 35도)
                                    delay = 5
                            elif TX_num == 36:          # 36 : 머리 왼쪽 90도 하향 30도
                                if hole_detected:
                                    ball_angle = -30    # 공을 찾음 -> 다음 사이클에서 ball_angle = -30 에 맞추어 3개의 동작 실행
                                    motion_cnt = 2
                                else:
                                    TX_num = 37         # 공을 못찾음 -> 다음 사이클에서 고개 더 내림 (하향 30도 -> 하향 45도)
                                    delay = 5
                            elif TX_num == 37:          # 37 : 머리 왼쪽 90도 하향 45도
                                if hole_detected:
                                    ball_angle = -45    # 다음 사이클에서 ball_angle = -45 에 맞추어 3개의 동작 실행
                                    motion_cnt = 2
                                else:
                                    TX_num = 38         # 머리 중앙 하향 0도
                                    delay = 5
